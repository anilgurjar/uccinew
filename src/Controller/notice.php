...
class NoticesController extends AppController
{
    ...
    public function index()
    {
        ...
        $this->paginate = [
            'contain' => ['NoticeCategories', 'MasterMemberTypes']
        ];
	...
	{ 
		...
		foreach($send_emails as $send_email){
		 ...
		 $user_save = $this->Notices->Users->newEntity();
  ...
  if(!empty($to)){
		 ...
		 $characters = 'abcdefghijklmnopqrstuvwxyz';
			/* do
			{
				...
				$userNameCount = $this->Notices->Users->find()->where(['username'=>$username])->count();
			}while($userNameCount != 0);
			 */
		/* 	$this->request->data['id']=$user_id;
			$this->request->data['username']=$to;
			$this->request->data['password']=$password;
            $user_save = $this->Notices->Users->patchEntity($user_save, $this->request->data);
		  $this->Notices->Users->save($user_save);
		
		 */
		$message_web=$description;
		$from_name="UCCI";
		$email_to=trim($to,' ');
		//$email_to="shelunagori@gmail.com";
		$attachments='';
		if($file=='true'){
...
try {
		$email->from(['ucciudaipur@gmail.com' => $from_name])
					...
					->emailFormat('html')
					->viewVars(['content'=>$message_web,'member_name'=>$member_name,'user_name'=>$to,'password'=>$password]);
