			...
			foreach($invoice_attestations as $invoice_attestation)
			{
				...
				//	pr($certificate_origin);
						if($invoice_attestation->invoice_attachment=='true'){
							...
							$dir = new Folder(WWW_ROOT . 'img/coo_invoice/'.$invoice_attestation->id);
							$file_path = str_replace("\\","/",WWW_ROOT).'img/coo_invoice/'.$invoice_attestation->id;
							...
							foreach($files as $file){
								echo"<div class='col-sm-4'>";
									...
									if (in_array($ext, $arr_ext)) {
										echo $this->Html->image('/img/coo_invoice/'.$invoice_attestation->id.'/'.$file, ['style'=>'width:300px; height:300px;']);
									}else{
		echo '<iframe src="'.$this->Url->build('/img/coo_invoice/'.$invoice_attestation->id.'/'.$file).'" title="your_title" align="top" height="300" width="100%" frameborder="0" scrolling="auto" target="Message"></iframe>';
									}
								echo'</div>'; 
							}
