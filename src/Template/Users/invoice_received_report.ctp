...
<div class="col-md-12">
	<div class="box box-primary">
		<div class="box-header with-border no-print">
			<h3 class="box-title">Payment Received Report</h3>
		</div>
		<div class="box-body">
	...
	<div class="table-responsive no-padding">
		<?php  
					echo $this->Html->link('<i class="fa fa-download"></i> Export',
						['controller' => 'Users', 'action' => 'invoiceReceivedExport'],
		...
		<table class="table table-borderd" style="width: 100%;border: #00c0ef; border-spacing: 0;border-collapse: collapse;" >
			...
			<tbody>
			<?php
			$page_no=$this->Paginator->current('Companies'); $sr_no=($page_no-1)*50;
			 foreach($master_member as $data) {
				// pr($data->users[0]->member_name); exit;
			 ...
			 ?>
 				<tr>
					...
					<td style="text-align:right"><?= h($data->total) ?></td>
				</tr>
		<?php } ?>
			</tbody>
		</table>
