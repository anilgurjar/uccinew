...
if(!empty($master_member))
{
	...
	if(!empty($master_membership_fee))
	{
		...
		{
			$master_turn_over=$this->requestAction(['controller'=>'Users', 'action'=>'FetchMasterTurnOvers'],['pass'=>array($member_data->user->turn_over_id)]);
		}
	}
	$company=$member_data->user->company_organisation;
	$address=$member_data->user->address;
	$city=$member_data->user->city;
	$pincode=$member_data->user->pincode;
...
<td style="padding-left:1px; ">
					...
					<td width="60%">
						<table width="100%">
							...
							<tr>
								...
								<td width="5%"><strong style="font-size:13px;">:</strong></td>
								<td width="65%" style="font-size:13px;">'.$email_to.'</td>
							</tr>';
	...
	Authorised Signatory</p>
			...
			$html.='</body></html>';
	if(!empty($master_turn_over))
	{
		unset($master_turn_over);
	}
...
$subject="Payment Reminder for the Financial Year 2017-2018";
$from_name="UCCI";
...
try {
	$email->from(['ucciudaipur@gmail.com' => $from_name])
			...
			->subject($subject)
			->profile('default')
			 ->template('performa_invoice_reminder')
			->emailFormat('html')
	...
	$this->requestAction(['controller'=>'Users', 'action'=>'UpdateInvoiceReminderMail'],['pass'=>array($invoice_id,2)]);
} catch (Exception $e) {

	echo 'Exception : ',  $e->getMessage(), "\n";
	$this->requestAction(['controller'=>'Users', 'action'=>'UpdateInvoiceReminderMail'],['pass'=>array($invoice_id,0)]);
