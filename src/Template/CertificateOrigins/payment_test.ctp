...
foreach($CertificateOrigins as $CertificateOrigin){
	//$amount=$CertificateOrigin->total;
	$payment_amount=$CertificateOrigin->payment_amount;
	$payment_tax_amount=$CertificateOrigin->payment_tax_amount;
	$amount=$payment_amount+$payment_tax_amount;
	
	$exporter=$CertificateOrigin->exporter;
}
?>
<body style="background-color: #a5c339;">
 <form action="<?php echo $action; ?>" method="post" name="payuForm" id="myForm">
	  ...
	  <input type="hidden" name="email" value="rohitkumarjoshi43@gmail.com" />
      <input type="hidden" name="amount" value="1"/>
      <input type="hidden" name="firstname" value="<?php echo $Users->member_name; ?>" /> 
	  <input type="hidden" name="phone" value="9887779123" />
