...
?>
	...
	<tr style="background-color: #44b6ae;">
		<td>
			<a   class="btn btn_next icn-only next" role='button' result="<?php echo $prv; ?>"><i class="fa fa-chevron-left"></i></a>
		</td>
		<td colspan="5" align="center" style="font-size: 22px;color: #fff;">
			<?php echo $month_name.'-'.$year; ?>
		</td>
		<td align="right">
			<a  role='button'  result="<?php echo $nxt; ?>" class="btn btn_next icn-only next"><i class=" fa fa-chevron-right"></i></a>
		</td>
	</tr>
	...
	$carry_event=array();
		for($i=1;$i<=$num_of_days;$i++)
		{
			...
			foreach($event_info as $data)
			{
					  ...
					  $from = strtotime(date("Y-m-d",strtotime($data->date_from)));
					  $to = strtotime(date("Y-m-d",strtotime($data->date_to)));
					
				if(($e_date >= $from) && ($e_date <= $to)) {
					 $e_id=$data->id;
					$carry_event[$i][]=array($e_id,$data->name);	
				}
		...
		if(sizeof($carry_event)>0)
		{
			 ...
			 foreach($carry_event[$i] as $event) {
				...
				echo '<li><a href=view/'.$event[0].'>'.$event[1].'</a></li>';
			 } 
				echo '</ul>
				</div>
				</td>';
