...
foreach($master_member_receipt as $data){
	...
	foreach($data->general_receipt_purposes as $purpose)
	{
		...
		$purpose_email[]=$purpose->master_purpose->email;
	}
...
<br/>
				...
				<br/>
					...
					Received with thanks from '.$data->user->company_organisation.', '.$data->user->city.'
					...
					$html.='. </p>
						...
						<table style="width: 100%;" class="border_none">
							<tr>';
							 ...
							 if($typeee == 1){ 
								  $html.='<td rowspan="2" style="width: 45%; text-align:left;">
									...
									$html.='</tr>
									<tr>
									<td style=" text-align:right;font-size: 15px;"><br/><br/><br/><br/>Authorised Signatory</td>
									</tr>';
								  }else{ 
								...
								$html.='</tr>
									...
									<td style="width: 70%; text-align:right;font-size: 15px;"><br/><br/><br/><br/>Authorised Signatory</td>
									</tr>';
								...
								$html.='</table></td></tr><tr><td><span style="width: 100%;font-size: 15px;line:height:2;">Note: Cheque/DD subject to clearance in bank.</span></td></tr></table></body></html>';  
...
file_put_contents('GeneralReceipt.pdf', $output);	

$email = new Email();
$email->transport('SendGrid');

$reply="ashishbohara1008@gmail.com";
$subject="General Receipt";
$message_web="General Receipt";
$from_name="UCCI";
$email_to=trim($email_to,' ');

try {
	$email->from(['ashishbohara1008@gmail.com' => $from_name])
				->to($email_to)
				->cc($purpose_email)
				->subject($subject)
				->attachments([
				'GeneralReceipt.pdf' => [
					'file' => 'GeneralReceipt.pdf'
				]
			])
			->send($message_web);
	
	$this->requestAction(['controller'=>'MemberReceipts', 'action'=>'UpdateReceiptMail'],['pass'=>array($receipt_id)]);
} catch (Exception $e) {
