<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Grievance Issue'), ['action' => 'edit', $grievanceIssue->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Grievance Issue'), ['action' => 'delete', $grievanceIssue->id], ['confirm' => __('Are you sure you want to delete # {0}?', $grievanceIssue->id)]) ?> </li>
...
<div class="grievanceIssues view large-9 medium-8 columns content">
    <h3><?= h($grievanceIssue->name) ?></h3>
    ...
    <div class="related">
        <h4><?= __('Related Grievance Issue Relateds') ?></h4>
        <?php if (!empty($grievanceIssue->grievance_issue_relateds)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($grievanceIssue->grievance_issue_relateds as $grievanceIssueRelateds): ?>
            <tr>
                <td><?= h($grievanceIssueRelateds->id) ?></td>
                <td><?= h($grievanceIssueRelateds->grievance_issue_id) ?></td>
                <td><?= h($grievanceIssueRelateds->name) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => 'GrievanceIssueRelateds', 'action' => 'view', $grievanceIssueRelateds->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'GrievanceIssueRelateds', 'action' => 'edit', $grievanceIssueRelateds->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'GrievanceIssueRelateds', 'action' => 'delete', $grievanceIssueRelateds->id], ['confirm' => __('Are you sure you want to delete # {0}?', $grievanceIssueRelateds->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
    ...
    <div class="related">
        ...
        <?php if (!empty($grievanceIssue->industrial_grievances)): ?>
        <table cellpadding="0" cellspacing="0">
            <tr>
                <th scope="col"><?= __('Id') ?></th>
            ...
            <?php foreach ($grievanceIssue->industrial_grievances as $industrialGrievances): ?>
            <tr>
